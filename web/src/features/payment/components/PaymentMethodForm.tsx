...
export function PaymentMethodForm() {
  ...
  const handlePaymentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const actualPaymentMethod = paymentMethod === 'wallet' ? walletType : paymentMethod;
      const _res = await api.post('/bookings/create', {
        tripScheduleId: tripId,
        seatNumbers: seats,
        paymentMethod: actualPaymentMethod,
        passengers: seats.map((_seat: string, idx: number) => ({
  ...
  return (
    <div className="lg:col-span-7">
      ...
      <form className="space-y-6" onSubmit={handlePaymentSubmit}>
        ...
        {/* Wallet Details Form */}
        <div 
          className={`bg-surface-container-lowest p-6 md:p-8 rounded-xl border border-outline-variant shadow-level-2 space-y-4 transition-all duration-300 ${paymentMethod !== 'wallet' ? 'hidden' : ''}`}
        >
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-label-md text-on-surface">Select E-Wallet</h3>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <label className="cursor-pointer">
              <input 
              ...
              />
              <div className="flex flex-col items-center justify-center p-4 border border-outline-variant rounded-xl peer-checked:border-primary peer-checked:bg-primary-fixed/30 transition-all hover:bg-surface-container">
                <img src="https://upload.wikimedia.org/wikipedia/vi/f/fe/MoMo_Logo.png" alt="MoMo" className="h-8 mb-2 object-contain" />
                <span className="font-label-md">MoMo</span>
              </div>
            </label>
            <label className="cursor-pointer">
              <input 
              ...
              />
              <div className="flex flex-col items-center justify-center p-4 border border-outline-variant rounded-xl peer-checked:border-primary peer-checked:bg-primary-fixed/30 transition-all hover:bg-surface-container">
                <img src="https://cdn.haitrieu.com/wp-content/uploads/2022/10/Logo-ZaloPay-Square.png" alt="ZaloPay" className="h-8 mb-2 object-contain" />
                <span className="font-label-md">ZaloPay</span>
              </div>
            </label>
          </div>
        </div>
